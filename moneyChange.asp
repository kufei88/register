...
<body>
    ...
    <div class="easyui-panel" title="充值" style="width:400px">
        ...
        <form id="ff" method="post">
            <table cellpadding="5">
                ...
                <tr>
                    <td>调整差额:</td>
                    <td><input class="easyui-numberbox" type="text" id="recharge" name="recharge" value="0"    ></input></td>
                </tr>
                <tr>
                    <td>调整后余额:</td>
                    <td><input class="easyui-numberbox" type="text" id="charge" name="charge" value="0"    ></input></td>
                </tr>
                <tr>
                    <td>备注:</td>
                    <td><input class="easyui-textbox" type="text" id="remark" name="remark"     ></input></td>
                </tr>
            </table>
        </form>
	...
	function submitForm(){
		...
		if (money !=''){
		$.messager.confirm('系统提示', '您确定要调整'+money+'吗?', function(r) {
			$.post('addMoneyChange.asp',{username:$('#company').combobox('getValue'),rechargemoney:$('#recharge').val(),
										remark:$('#remark').val()},
								function(result){
	...
	onSelect: function(record){
		...
		$('#charge').numberbox('setValue',parseFloat(record.jsmoney)+parseFloat($('#charge').numberbox('getValue')));
	}
	
});
	$('#recharge').numberbox({
		onChange:function(newValue,oldValue){
			$('#charge').numberbox('setValue',parseFloat($('#jsmoney').numberbox('getValue'))+parseFloat(newValue));
		}
	});
