...
<%
	...
	if rsUser.BOF and rsUser.EOF then
	  sql = "insert into  jsphoneuuid (company,[user],uuid,date,remark,agent) values ('"_
	  &company&"','"&User&"','"&uuid&"',getdate(),'"&remark&"','"&agent&"')"
	  conn.execute(sql)
	  rsUser.close
	  sql = "select (case when convert(datetime,convert(char(4),year(getdate()))+'-'+memvalue)>getdate() "_
		...
		rsUser.close
		sql = "select convert(char(10),isnull(begindate,'2016-01-01'),120) as begindate from agent where username='"&agent&"'"
		rsUser.Open sql,conn,0,1
